...
if not firebase_admin._apps:
    cred = credentials.Certificate(
        "ugc-net-marks-f2f20-firebase-adminsdk-fbsvc-cadfc44bd2.json"
    )
...
def process_url(url):
    ...
    p1_marks = p1_correct * 2
    p2_marks = p2_correct * 2
    total_marks = p1_marks + p2_marks

    return application, candidate_name, roll_no, p1_correct, p2_correct, total_marks, df,p1_marks,p2_marks
...
if st.button("Get marks "):
    ...
    if result:
        app, name, roll, p1c, p2c, total, df,p1_marks,p2_marks = result
        ...
        st.markdown(
            ...
            <b>Name:</b> {name}<br>
            <b>Roll No:</b> {roll}<br>
            <b>Application No:</b> {app}<br>
            <b>Total Marks:</b> {total}<br>
	    <b>Paper1 Marks:</b> {p1_marks}<br>
	    <b>Paper2 Marks:</b> {p2_marks}<br>
            </p><hr>
            """,
        ...
        with col1:
            st.plotly_chart(
                animated_pie("Paper 1", p1c, PAPER1_COUNT-p1c,
                             ["Green", "Red"]),
        ...
        with col2:
            st.plotly_chart(
                animated_pie("Paper 2", p2c, PAPER2_COUNT-p2c,
                             ["Green", "Red"]),
                use_container_width=True
            )
        ...
        with col3:
            st.plotly_chart(
                animated_pie("Combined", p1c+p2c,
                             (PAPER1_COUNT+PAPER2_COUNT)-(p1c+p2c),
                             ["Green", "Red"]),
                use_container_width=True
            )
        ...
        if saved:
            st.success("✅ Thank u for using ")
        else:
            st.info("Thank u for using")

        csv = df.to_csv(index=False).encode("utf-8")
        st.download_button("⬇️ Download CSV", csv, "ugc_net_result.csv", "text/csv")
